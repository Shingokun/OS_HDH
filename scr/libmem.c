...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  cur_vma->sbrk+=inc_sz;
  struct vm_rg_struct new;
  new.rg_start=old_sbrk;
  new.rg_end=cur_vma->sbrk;
  new.rg_next=NULL;
  enlist_vm_freerg_list(caller->mm, &new);
...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  if (region == NULL)
    return -1;
  rgnode.rg_start=region->rg_start;
  rgnode.rg_end=region->rg_end;
  rgnode.rg_next=NULL;
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    /* Update page table */
    int swpoff = swpfpn * PAGING_PAGESZ; 
    pte_set_swap(&mm->pgd[vicpgn], 0, swpoff);
    //mm->pgd;
