 ...
 */
 int vmap_page_range(struct pcb_t *caller, int addr, int pgnum, struct framephy_struct *frames, struct vm_rg_struct *ret_rg)
 {
     ...
     for (pgit = 0; pgit < pgnum && fpit != NULL; pgit++) {
         /* Cập nhật bảng trang */
         if (pte_set_fpn(&caller->mm->pgd[pgn + pgit], fpit->fpn) < 0) {
             return -1; /* Thiết lập PTE thất bại */
         }
 
         /* Theo dõi trang được sử dụng */
         enlist_pgn_node(&caller->mm->fifo_pgn, pgn + pgit);
 
         /* Cập nhật owner của khung */
     ...
     *frm_lst = NULL;
     ...
     for (pgit = 0; pgit < req_pgnum; pgit++) {
         if (MEMPHY_get_freefp(caller->mram, &fpn) == 0) {
             ...
             /* Không đủ khung, trả lại các khung đã cấp */
             while (*frm_lst != NULL) {
                 newfp_str = *frm_lst;
 ...
 */
 int init_mm(struct mm_struct *mm, struct pcb_t *caller)
 {
     ...
     memset(mm->pgd, 0, PAGING_MAX_PGN * sizeof(uint32_t)); /* Xóa bảng trang */
 
     /* Khởi tạo VMA0 */
     vma0->vm_id = 0;
     vma0->vm_start = 0;
     vma0->vm_end = PAGING_SBRK_INIT_SZ; /* Kích thước ban đầu */
